...
This feature loads **[SpriteCollab](https://sprites.pmdcollab.org)** portraits for those evolved forms from `graphics/portraits/<species>/`, converts them at build time to **AT4PN** + RGBX palettes, and serves them from a separate `pksdir0` archive when enabled.
...
| `*.pal` / `*.at4pn` | Build outputs next to each PNG |
