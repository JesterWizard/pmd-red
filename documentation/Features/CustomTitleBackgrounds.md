...
|------|--------|-----------|
| `TRUE` (default) | `title_01.png` … `title_06.png` | 8bpp BG3, up to ~239 colors |
...
4. Fade in the 240-entry palette; main menu runs as usual.
5. On title exit, `SetTitleBg8bpp(FALSE)` restores vanilla BG control.

### Title-only VRAM layout

Vanilla maps sit at screenbases 12–15 (`0x6000`–`0x7FFF`) and UI chrome (font sheet) at `0x4F00` (tiles `0x278+`). A full 8bpp image needs more tile VRAM than CHARBASE2 alone can give without colliding with those regions.

| Region | Title 8bpp mode | Purpose |
|--------|-----------------|--------|
| `0x0000`–`0x1FFF` | Window / UI 4bpp tiles (CHARBASE 0) | Menu text & window fill GFX |
| `0x2000`–`0x3FFF` | Screenbases **4–7** | BG0–3 tilemaps |
| `0x4F00`–`0x5FFF` | Font / window chrome (untouched) | Borders, fill tiles `0x278+` |
| `0x6000`–`0xFFFF` | 8bpp title tiles | CHARBASE1 indices **128…767** (max **640** tiles) |

BG3CNT while active: `BGCNT_256COLOR | CHARBASE(1) | SCREENBASE(7)`.
...
| Colors | ≤239 opaque | Palette index **0** reserved (GBA transparent) |
| Palette file | 240 RGBX | `unk4=0x80`; fade loop loads indices 0–239 |
| Unique tiles | ≤640 | Flip-aware; extras nearest-merged |
...
- Stay under ~640 unique 8×8 tiles after flips (current set is ~562–601). Heavy noise/dither raises tile count and may force merges.
...
| BG registers | `UpdateBGControlRegisters` in `src/reg_control.c` | Screenbases 4–7 + BG3 256-color / CHARBASE1 |
| Tilemap DMA | `DoScheduledMemCopies` / `CopyBgTilemaps0And1` in `src/graphics_memory.c` | Copies to bases 4–7 when flag set |
...
- **640-tile cap** (CHARBASE1 from index 128). Images that exceed it are similarity-merged and can show soft 8×8 mismatches.
- **Palette 0–239** is owned by the title fade; UI chrome uses bank 15 (240–255). Do not extend the title fade into font colors.
...
If something looks wrong (garbled UI, black left strip, scrambled tiles), check that the font region at `0x4F00` was not overwritten and that tile indices were offset by 128.
