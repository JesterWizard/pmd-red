...
- **Wide UI (dynamic):** `ShowWindows` counts the window stack. If it needs more than 256 tiles (Team list+action, Summary, …), `gGroundMap8bppWideUi` hides BG2/BG3 and loads window tile gfx from **tile 2** into low VRAM (`0x06000000+`, reclaiming art map SB 0/1 — same as vanilla UI). UI maps stay at **SB 6/7** (~382 tile budget; also keeps `sWindowGFXPool` indices in range). Closing restores explore layout (tile 128+) and invalidates the art streamer. Dialogue width / Toolbox row caps still avoid flipping for small boxes.
