...
void sub_8001D88(void)
{
    ...
    if ((ScriptVarScenarioEqual(SCENARIO_SUB1,0x1f,0)) && (GetFriendAreaUnlocked(FRIEND_AREA_SKY_BLUE_PLAINS))) {
    ...
    if (sMain_main > 0x11) {
        ...
        if (GetFriendAreaUnlocked(FRIEND_AREA_FURNACE_DESERT) != 0) {
            sub_80973A8(RESCUE_DUNGEON_DESERT_REGION, TRUE);
        }
        if (GetFriendAreaUnlocked(FRIEND_AREA_BOULDER_CAVE)) {
            sub_80973A8(RESCUE_DUNGEON_SOUTHERN_CAVERN, TRUE);
        }
        if (GetFriendAreaUnlocked(FRIEND_AREA_DRAGON_CAVE)) {
            sub_80973A8(RESCUE_DUNGEON_WYVERN_HILL, TRUE);
        }
        if (GetFriendAreaUnlocked(FRIEND_AREA_SECRETIVE_FOREST)) {
            sub_80973A8(RESCUE_DUNGEON_DARKNIGHT_RELIC, TRUE);
        }
        if (GetFriendAreaUnlocked(FRIEND_AREA_SERENE_SEA)) {
            sub_80973A8(RESCUE_DUNGEON_GRAND_SEA, TRUE);
            sub_80973A8(RESCUE_DUNGEON_FAR_OFF_SEA, TRUE);
        }
        if (GetFriendAreaUnlocked(FRIEND_AREA_AGED_CHAMBER_AN) && GetFriendAreaUnlocked(FRIEND_AREA_AGED_CHAMBER_O_EXCLAIM)) {
        ...
        if ((FindItemInInventory(ITEM_HM_SURF) != -1) || (gTeamInventoryRef->teamStorage[ITEM_HM_SURF] != 0)) {
            ...
            if ((ScriptVarScenarioEqual(SCENARIO_SUB6,0,0)) && (GetFriendAreaUnlocked(FRIEND_AREA_SOUTHERN_ISLAND))) {
        ...
        if (!ScriptVarScenarioBefore(SCENARIO_SUB6,0x30,0)) {
            ...
            if ((ScriptVarScenarioEqual(SCENARIO_SUB8,0,0) && (ScriptVarScenarioAfter(SCENARIO_SUB2,0x21,3))) &&
                 (GetFriendAreaUnlocked(FRIEND_AREA_SKY_BLUE_PLAINS))) {
                ScenarioCalc(SCENARIO_SUB8,0x33,1);
            }
            if (GetFriendAreaUnlocked(FRIEND_AREA_SKY_BLUE_PLAINS)) {
