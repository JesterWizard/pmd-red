...
const u8 *const gAbilityDescriptions[NUM_ABILITIES] = {
    [ABILITY_UNKNOWN] = _("{color CYAN}-------"),
    [ABILITY_STENCH] = _("{color CYAN}Stench{reset}: May terrify an\nattacking foe."),
    [ABILITY_THICK_FAT] = _("{color CYAN}Thick Fat{reset}: Resistant to\nFire- and Ice-type moves."),
    [ABILITY_RAIN_DISH] = _("{color CYAN}Rain Dish{reset}: Improves HP gain\nif it is raining."),
    [ABILITY_DRIZZLE] = _("{color CYAN}Drizzle{reset}: Changes the\nweather to rainy."),
    [ABILITY_ARENA_TRAP] = _("{color CYAN}Arena Trap{reset}: Immobilizes any\nattacker except Flying-type\nfoes and those with Levitate."),
    [ABILITY_INTIMIDATE] = _("{color CYAN}Intimidate{reset}: Lowers the\nPhysical Attack power of\nattackers."),
    [ABILITY_ROCK_HEAD] = _("{color CYAN}Rock Head{reset}: Prevents damage\nfrom the recoil of moves\nsuch as Take Down."),
    [ABILITY_AIR_LOCK] = _("{color CYAN}Air Lock{reset}: Changes the\nweather to clear."),
    [ABILITY_HYPER_CUTTER] = _("{color CYAN}Hyper Cutter{reset}: Prevents\nAttack from being lowered."),
    [ABILITY_SHADOW_TAG] = _("{color CYAN}Shadow Tag{reset}: Immobilizes\nattackers."),
    [ABILITY_SPEED_BOOST] = _("{color CYAN}Speed Boost{reset}: The Pokémon's\nMovement Speed accelerates."),
    [ABILITY_BATTLE_ARMOR] = _("{color CYAN}Battle Armor{reset}: Blocks\ncritical hits."),
    [ABILITY_STURDY] = _("{color CYAN}Sturdy{reset}: Blocks\none-hit KO moves."),
    [ABILITY_SUCTION_CUPS] = _("{color CYAN}Suction Cups{reset}: Prevents\nforced movement."),
    [ABILITY_CLEAR_BODY] = _("{color CYAN}Clear Body{reset}: Prevents\nreduction of stats."),
    [ABILITY_TORRENT] = _("{color CYAN}Torrent{reset}: Boosts the power\nof Water-type moves when\nHP is low."),
    [ABILITY_GUTS] = _("{color CYAN}Guts{reset}: Boosts Attack if the\nPokémon has any status\nproblems."),
    [ABILITY_ROUGH_SKIN] = _("{color CYAN}Rough Skin{reset}: Inflicts damage\non attackers."),
    [ABILITY_SHELL_ARMOR] = _("{color CYAN}Shell Armor{reset}: Blocks\ncritical hits."),
    [ABILITY_NATURAL_CURE] = _("{color CYAN}Natural Cure{reset}: Recovers\nquickly from status\nproblems."),
    [ABILITY_DAMP] = _("{color CYAN}Damp{reset}: Prevents any\nexplosions on the dungeon\nfloor."),
    [ABILITY_LIMBER] = _("{color CYAN}Limber{reset}: Prevents paralysis."),
    [ABILITY_MAGNET_PULL] = _("{color CYAN}Magnet Pull{reset}: Immobilizes\nSteel-type attackers."),
    [ABILITY_WHITE_SMOKE] = _("{color CYAN}White Smoke{reset}: Prevents\nreduction of stats."),
    [ABILITY_SYNCHRONIZE] = _("{color CYAN}Synchronize{reset}: Shares\nstatus problems with any\nPokémon in reach."),
    [ABILITY_OVERGROW] = _("{color CYAN}Overgrow{reset}: Boosts the power\nof Grass-type moves when\nHP is low."),
    [ABILITY_SWIFT_SWIM] = _("{color CYAN}Swift Swim{reset}: Boosts attack\nfrequency if it is raining."),
    [ABILITY_SAND_STREAM] = _("{color CYAN}Sand Stream{reset}: Changes the\nweather to a sandstorm."),
    [ABILITY_SAND_VEIL] = _("{color CYAN}Sand Veil{reset}: Boosts Evasion\nif there is a sandstorm."),
    [ABILITY_KEEN_EYE] = _("{color CYAN}Keen Eye{reset}: Prevents Accuracy\nfrom going down."),
    [ABILITY_INNER_FOCUS] = _("{color CYAN}Inner Focus{reset}: Prevents\ncringing."),
    [ABILITY_STATIC] = _("{color CYAN}Static{reset}: Paralyzes attackers."),
    [ABILITY_SHED_SKIN] = _("{color CYAN}Shed Skin{reset}: Recovers easily\nfrom status problems."),
    [ABILITY_HUGE_POWER] = _("{color CYAN}Huge Power{reset}: Occasionally\nboosts Attack."),
    [ABILITY_VOLT_ABSORB] = _("{color CYAN}Volt Absorb{reset}: Restores HP\nif damaged by an Electric-\ntype move."),
    [ABILITY_WATER_ABSORB] = _("{color CYAN}Water Absorb{reset}: Restores HP\nif hit by a Water-type move."),
    [ABILITY_FORECAST] = _("{color CYAN}Forecast{reset}: The Pokémon's\ntype changes with the\nweather."),
    [ABILITY_SERENE_GRACE] = _("{color CYAN}Serene Grace{reset}: Improves the\nchances of triggering the\nadded effects of moves."),
    [ABILITY_POISON_POINT] = _("{color CYAN}Poison Point{reset}: Poisons\nattackers."),
    [ABILITY_TRACE] = _("{color CYAN}Trace{reset}: Changes ability to\nthat of the foe attacking\nor using a move."),
    [ABILITY_OBLIVIOUS] = _("{color CYAN}Oblivious{reset}: Prevents\ninfatuation."),
    [ABILITY_TRUANT] = _("{color CYAN}Truant{reset}: Prevents action in\nyour next turn after using\na move or a Wonder Orb."),
    [ABILITY_RUN_AWAY] = _("{color CYAN}Run Away{reset}: Becomes terrified\nand flees when HP gets low.\nLeaders will resist this urge."),
    [ABILITY_STICKY_HOLD] = _("{color CYAN}Sticky Hold{reset}: Prevents the\nPokémon's hold item from\nbeing taken."),
    [ABILITY_CLOUD_NINE] = _("{color CYAN}Cloud Nine{reset}: Changes the\nweather to clear."),
    [ABILITY_ILLUMINATE] = _("{color CYAN}Illuminate{reset}: If damaged,\na wild Pokémon appears \non the dungeon floor."),
    [ABILITY_EARLY_BIRD] = _("{color CYAN}Early Bird{reset}: Awakens quickly\nfrom sleep."),
    [ABILITY_HUSTLE] = _("{color CYAN}Hustle{reset}: Boosts Attack but\nlowers Accuracy."),
    [ABILITY_DROUGHT] = _("{color CYAN}Drought{reset}: Changes the\nweather to sunny."),
    [ABILITY_LIGHTNINGROD] = _("{color CYAN}Lightningrod{reset}: Attracts\nElectric-type moves."),
    [ABILITY_COMPOUNDEYES] = _("{color CYAN}Compoundeyes{reset}: Boosts\nthe Accuracy of moves."),
    [ABILITY_MARVEL_SCALE] = _("{color CYAN}Marvel Scale{reset}: Boosts\nDefense if there is a status\nproblem."),
    [ABILITY_WONDER_GUARD] = _("{color CYAN}Wonder Guard{reset}: Only\nsuper-effective moves can\ninflict damage."),
    [ABILITY_INSOMNIA] = _("{color CYAN}Insomnia{reset}: Prevents sleep."),
    [ABILITY_LEVITATE] = _("{color CYAN}Levitate{reset}: Blocks all\nGround-type moves."),
    [ABILITY_PLUS] = _("{color CYAN}Plus{reset}: Boosts Special Attack\nif there is a Pokémon with\nthe Minus ability."),
    [ABILITY_PRESSURE] = _("{color CYAN}Pressure{reset}: If hit by a foe's\nmove, the foe uses more PP\nthan usual."),
    [ABILITY_LIQUID_OOZE] = _("{color CYAN}Liquid Ooze{reset}: Inflicts damage\non the foe if hit by a move\nthat drains HP."),
    [ABILITY_COLOR_CHANGE] = _("{color CYAN}Color Change{reset}: Changes the\nPokémon's type to that of\nthe move that hit it last."),
    [ABILITY_SOUNDPROOF] = _("{color CYAN}Soundproof{reset}: Blocks the\neffects of all moves that\nuse sound."),
    [ABILITY_EFFECT_SPORE] = _("{color CYAN}Effect Spore{reset}: Attackers are\npoisoned, paralyzed, or put\nto sleep."),
    [ABILITY_FLAME_BODY] = _("{color CYAN}Flame Body{reset}: Afflicts\nattackers with burns."),
    [ABILITY_MINUS] = _("{color CYAN}Minus{reset}: Boosts Special Attack\nif there is a Pokémon with\nthe Plus ability."),
    [ABILITY_OWN_TEMPO] = _("{color CYAN}Own Tempo{reset}: Prevents\nconfusion."),
    [ABILITY_MAGMA_ARMOR] = _("{color CYAN}Magma Armor{reset}: Prevents\nfreezing."),
    [ABILITY_WATER_VEIL] = _("{color CYAN}Water Veil{reset}: Prevents burns."),
    [ABILITY_SWARM] = _("{color CYAN}Swarm{reset}: Boosts the power\nof Bug-type moves when\nHP is low."),
    [ABILITY_CUTE_CHARM] = _("{color CYAN}Cute Charm{reset}: Causes\nattackers to become\ninfatuated."),
    [ABILITY_IMMUNITY] = _("{color CYAN}Immunity{reset}: Prevents\nbecoming poisoned or badly\npoisoned."),
    [ABILITY_BLAZE] = _("{color CYAN}Blaze{reset}: Boosts the power\nof Fire-type moves when\nHP is low."),
    [ABILITY_PICKUP] = _("{color CYAN}Pickup{reset}: Finds items if\nthe Pokémon is not holding\nanything."),
    [ABILITY_FLASH_FIRE] = _("{color CYAN}Flash Fire{reset}: Zeroes damage\nfrom Fire-type move hits,\nand boosts own Fire moves."),
    [ABILITY_VITAL_SPIRIT] = _("{color CYAN}Vital Spirit{reset}: Prevents sleep."),
    [ABILITY_CHLOROPHYLL] = _("{color CYAN}Chlorophyll{reset}: Boosts attack\nfrequency if the weather is\nsunny."),
    [ABILITY_PURE_POWER] = _("{color CYAN}Pure Power{reset}: Occasionally\nboosts Attack."),
    [ABILITY_SHIELD_DUST] = _("{color CYAN}Shield Dust{reset}: Blocks the added\neffects of moves."),
    [ABILITY_NORMALIZE] = _("{color CYAN}Normalize{reset}: Enables the Pokémon\nto use all of its moves as if\nthey were Normal-type moves."),
    [ABILITY_ADAPTABILITY] = _("{color CYAN}Adaptability{reset}: Boosts the power\nof the Pokémon's moves that\nmatch its type."),
    [ABILITY_AFTERMATH] = _("{color CYAN}Aftermath{reset}: Causes the\nPokémon to explode upon\nfainting."),
    [ABILITY_ANGER_POINT] = _("{color CYAN}Anger Point{reset}: Boosts the\nPokémon's Attack when the\nPokémon suffers a critical hit."),
    [ABILITY_ANTICIPATION] = _("{color CYAN}Anticipation{reset}: Shows if the\nfoe has a super-effective move\nupon hitting it with a regular attack."),
    [ABILITY_DOWNLOAD] = _("{color CYAN}Download{reset}: Boosts the\nPokémon's Attack or Special\nAttack when it attacks."),
    [ABILITY_DRY_SKIN] = _("{color CYAN}Dry Skin{reset}: Weakens Pokémon\nin sun and to Fire-type moves;\nrain and Water-type moves heal HP."),
    [ABILITY_FILTER] = _("{color CYAN}Filter{reset}: Reduces the damage\nthat the Pokémon suffers from\nsuper-effective moves."),
    [ABILITY_GLUTTONY] = _("{color CYAN}Gluttony{reset}: Speeds up the\nBelly's food consumption."),
    [ABILITY_ICE_BODY] = _("{color CYAN}Ice Body{reset}: Promotes natural\nHP recovery when it is hailing."),
    [ABILITY_IRON_FIST] = _("{color CYAN}Iron Fist{reset}: Boosts the power\nthe Pokémon uses for punching\nmoves."),
    [ABILITY_LEAF_GUARD] = _("{color CYAN}Leaf Guard{reset}: Protects the\nPokémon from status problems\nin sunny weather."),
    [ABILITY_MAGIC_GUARD] = _("{color CYAN}Magic Guard{reset}: Protects the\nPokémon from all damage except\nfrom direct attacks."),
    [ABILITY_MOLD_BREAKER] = _("{color CYAN}Mold Breaker{reset}: Prevents the\nfoe's ability from activating when\nthe Pokémon attacks."),
    [ABILITY_NO_GUARD] = _("{color CYAN}No Guard{reset}: Allows Pokémon to\nignore stat modifications to\naccuracy and evasiveness."),
    [ABILITY_POISON_HEAL] = _("{color CYAN}Poison Heal{reset}: Enables the\nPokémon to recover HP when\npoisoned rather than taking damage."),
    [ABILITY_QUICK_FEET] = _("{color CYAN}Quick Feet{reset}: Boosts the\nPokémon's Movement Speed when\nit has any status problems."),
    [ABILITY_RECKLESS] = _("{color CYAN}Reckless{reset}: Boosts the\nPokémon's power with recoiling\nattacks but also raises self-inflicted damage."),
    [ABILITY_RIVALRY] = _("{color CYAN}Rivalry{reset}: Boosts Pokémon's\nAttack when facing same gender\nand reduces it when not the same."),
    [ABILITY_SKILL_LINK] = _("{color CYAN}Skill Link{reset}: Enables the\nPokémon to always attempt the\ntop number of hits with multistrike moves."),
    [ABILITY_SNIPER] = _("{color CYAN}Sniper{reset}: Boosts the damage\nthe Pokémon inflicts with\ncritical hits."),
    [ABILITY_SNOW_CLOAK] = _("{color CYAN}Snow Cloak{reset}: Boosts the\nPokémon's evasiveness if it is\nhailing or snowing."),
    [ABILITY_SOLAR_POWER] = _("{color CYAN}Solar Power{reset}: Boosts Special\nAttack in sunny weather, though\nthe Pokémon also loses HP."),
    [ABILITY_SOLID_ROCK] = _("{color CYAN}Solid Rock{reset}: Reduces the\ndamage that the Pokémon suffers\nfrom super-effective attacks."),
    [ABILITY_STALL] = _("{color CYAN}Stall{reset}: Restricts the Pokémon\nto using moves on foes one tile\naway. Doesn't affect leaders."),
    [ABILITY_SUPER_LUCK] = _("{color CYAN}Super Luck{reset}: Improves the\nchances of the Pokémon making\ncritical hits."),
    [ABILITY_TANGLED_FEET] = _("{color CYAN}Tangled Feet{reset}: Boosts the\nPokémon's evasiveness when it is\nconfused or cross-eyed."),
    [ABILITY_TECHNICIAN] = _("{color CYAN}Technician{reset}: Boosts the\ndamage the Pokémon inflicts with\nmoves that have low attack power."),
    [ABILITY_TINTED_LENS] = _("{color CYAN}Tinted Lens{reset}: Boosts damage\nthat the Pokémon inflicts with\nmoves that are not very effective."),
    [ABILITY_TOXIC_BOOST] = _("{color CYAN}Toxic Boost{reset}: Boosts the\nPokémon's offensive power while\npoisoned."),
};
...
const u8 *const gTextStat = _("Stat");
const u8 *const gTextPhysical = _("Physical");
const u8 *const gTextSpecial = _("Special");
...
const u8 *const gText_ExpPtsUnkMacroCyan = _("Exp. Pts.: {UNK_MACRO_3E M3E_82}{color CYAN_RAW}{VALUE_1}{reset} ");
const u8 *const gText_ToNextLevel = _("To next level:{UNK_MACRO_3E M3E_82}{VALUE_0}");
const u8 *const gText_ToNextLevelCyan = _("To next level:{UNK_MACRO_3E M3E_82}{color CYAN_RAW}{VALUE_0}{reset}");
...
/* PMD2-style summary: 2x2 grid — left values @52, right labels @80, right values @128 */
const u8 *const gText_SummaryAttack = _("Attack:{UNK_MACRO_3E M3E_52}$v03");
const u8 *const gText_SummaryAttackBoost = _("Attack:{UNK_MACRO_3E M3E_52}{color RED}$v03{reset}");
const u8 *const gText_SummaryDefense = _("{UNK_MACRO_3E M3E_80}Defense:{UNK_MACRO_3E M3E_128}$v03");
const u8 *const gText_SummaryDefenseBoost = _("{UNK_MACRO_3E M3E_80}Defense:{UNK_MACRO_3E M3E_128}{color RED}$v03{reset}");
const u8 *const gText_SummarySpAtk = _("Sp. Atk.:{UNK_MACRO_3E M3E_52}$v03");
const u8 *const gText_SummarySpAtkBoost = _("Sp. Atk.:{UNK_MACRO_3E M3E_52}{color RED}$v03{reset}");
const u8 *const gText_SummarySpDef = _("{UNK_MACRO_3E M3E_80}Sp. Def.:{UNK_MACRO_3E M3E_128}$v03");
const u8 *const gText_SummarySpDefBoost = _("{UNK_MACRO_3E M3E_80}Sp. Def.:{UNK_MACRO_3E M3E_128}{color RED}$v03{reset}");
...
const u8 *const gText_SummaryTactic = _("Tactic:{UNK_MACRO_3E M3E_52}{POKEMON_0} ");
...
const u8 *const gIQSkillNames[NUM_IQ_SKILLS] = {
    ...
    [IQ_TIME_TRIPPER] = _("Time Tripper"),
    [IQ_NATURAL_GIFTER] = _("Natural Gifter"),
    [IQ_QUICK_STRIKER] = _("Quick Striker"),
    [IQ_PIERCE_HURLER] = _("Pierce Hurler"),
    [IQ_MAP_SURVEYOR] = _("Map Surveyor"),
    [IQ_INTIMIDATOR] = _("Intimidator"),
    [IQ_FAST_FRIEND] = _("Fast Friend"),
    [IQ_EXTRA_STRIKER] = _("Extra Striker"),
    [IQ_AGGRESSOR] = _("Aggressor"),
    [IQ_DEFENDER] = _("Defender"),
    [IQ_GAP_PROBER] = _("Gap Prober"),
    [IQ_PRACTICE_SWINGER] = _("Practice Swinger"),
    [IQ_WISE_HEALER] = _("Wise Healer"),
    [IQ_BRICK_TOUGH] = _("Brick Tough"),
    [IQ_ACUTE_SNIFFER] = _("Acute Sniffer"),
    [IQ_CHEERLEADER] = _("Cheerleader"),
    [IQ_BODYGUARD] = _("Bodyguard"),
};
...
const u8 *const gIQSkillDescriptions[NUM_IQ_SKILLS] = {
    ...
    [IQ_TIME_TRIPPER] = _("Raises Movement Speed by 1 while\nactive."),
    [IQ_NATURAL_GIFTER] = _("Extends the effects of consumed berries\nand seeds to the whole party.\nOnly works for the leader."),
    [IQ_QUICK_STRIKER] = _("Can do two regular attacks in one\nturn."),
    [IQ_PIERCE_HURLER] = _("Thrown items pierce Pokémon and walls\nand keep flying."),
    [IQ_MAP_SURVEYOR] = _("Knows the floor layout.\nDoes not reveal the stairs."),
    [IQ_INTIMIDATOR] = _("Sometimes makes a foe cringe when they\nattack."),
    [IQ_FAST_FRIEND] = _("Raises the chance of recruiting foes.\nOnly works for the leader."),
    [IQ_EXTRA_STRIKER] = _("Sometimes strikes again following a\nregular attack."),
    [IQ_AGGRESSOR] = _("Raises Attack and Sp. Atk by 1.\nLowers Defense and Sp. Def by 1."),
    [IQ_DEFENDER] = _("Raises Defense and Sp. Def by 1.\nLowers Attack and Sp. Atk by 1."),
    [IQ_GAP_PROBER] = _("Ranged moves and thrown items pass\nallies without causing damage.\n(Turn off Course Checker.)"),
    [IQ_PRACTICE_SWINGER] = _("Raises Attack and Sp. Atk by 1 for\nthe next turn after a move misses."),
    [IQ_WISE_HEALER] = _("Raises the effectiveness of\nHP-recovery moves and items."),
    [IQ_BRICK_TOUGH] = _("Permanently adds 10 to max HP.\nThis IQ Skill can't be turned off."),
    [IQ_ACUTE_SNIFFER] = _("Learns the number of items on the floor\nwhen entering a new floor.\nDoes not count items buried in walls."),
    [IQ_CHEERLEADER] = _("Raises Attack and Special Attack of\nfriends by 1."),
    [IQ_BODYGUARD] = _("Takes damage in place of friends who\nare low on {color YELLOW}HP{reset}."),
};
