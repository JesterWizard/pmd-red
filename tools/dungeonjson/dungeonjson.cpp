...
string generate_trap_table_text(Json map_data) {

    ostringstream text;
    int probability_total;

    text << "@ This is auto-generated by trap_found.json.\n";


    for (Json data_entry : map_data.array_items()) {
        probability_total = 0;

        text << ".global " << data_entry["name"].string_value() << "\n";
        text << data_entry["name"].string_value() << ":\n";

        text << ".2byte ";


        probability_total += data_entry["traps"]["Trip Trap"].int_value();
        if(data_entry["traps"]["Trip Trap"].int_value() == 0)
        {
            text <<  "0x00";
        }
        else
        {
            text <<  probability_total;
        }
        probability_total += data_entry["traps"]["Mud Trap"].int_value();
        if(data_entry["traps"]["Mud Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Sticky Trap"].int_value();
        if(data_entry["traps"]["Sticky Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Grimy Trap"].int_value();
        if(data_entry["traps"]["Grimy Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Summon Trap"].int_value();
        if(data_entry["traps"]["Summon Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Pitfall Trap"].int_value();
        if(data_entry["traps"]["Pitfall Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Warp Trap"].int_value();
        if(data_entry["traps"]["Warp Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Gust Trap"].int_value();
        if(data_entry["traps"]["Gust Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Spin Trap"].int_value();
        if(data_entry["traps"]["Spin Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Slumber Trap"].int_value();
        if(data_entry["traps"]["Slumber Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Slow Trap"].int_value();
        if(data_entry["traps"]["Slow Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Seal Trap"].int_value();
        if(data_entry["traps"]["Seal Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Poison Trap"].int_value();
        if(data_entry["traps"]["Poison Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Selfdestruct Trap"].int_value();
        if(data_entry["traps"]["Selfdestruct Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Explosion Trap"].int_value();
        if(data_entry["traps"]["Explosion Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["PP-Zero Trap"].int_value();
        if(data_entry["traps"]["PP-Zero Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Chestnut Trap"].int_value();
        if(data_entry["traps"]["Chestnut Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Wonder Tile"].int_value();
        if(data_entry["traps"]["Wonder Tile"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Pokemon Trap"].int_value();
        if(data_entry["traps"]["Pokemon Trap"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
        ...
        probability_total += data_entry["traps"]["Spiked Tile"].int_value();
        if(data_entry["traps"]["Spiked Tile"].int_value() == 0)
        {
            text << ", " <<  "0x00";
        }
        else
...
string generate_dungeon_item_encodings_text(Json map_data) {

    const int ITEM_SETS_SKIP_NUMBER = 30000;
    ...
    for (Json data_entry : map_data.array_items()) {
        ...
        for (Json category_entry : data_entry["categories"].array_items()) {

            item_probability = 0;

            this_entry.index = get_item_id(category_entry["category"].string_value());
            this_entry.index_name = category_entry["category"].string_value();
            this_entry.probability = category_entry["categoryProbability"].int_value() + category_probability;
            entries.push_back(this_entry);
            category_probability += category_entry["categoryProbability"].int_value();

            for (Json item_entry : category_entry["items"].array_items()) {

                this_entry.index = get_item_id(item_entry["item"].string_value());
                this_entry.index_name = item_entry["item"].string_value();
                this_entry.probability = item_entry["itemProbability"].int_value() + item_probability;
                entries.push_back(this_entry);
                item_probability += item_entry["itemProbability"].int_value();
            }
        }
        ...
        for (entry e : entries) {
            ...
            if(e.index - last_index)
            {
                ...
                text << ITEM_SETS_SKIP_NUMBER + e.index - last_index;
            }
            ...
            text << e.probability;
            last_index = e.index + 1;
        }

        entries.clear();
        text << ", " << ITEM_SETS_SKIP_NUMBER + get_num_item_ids() - last_index << "\n";
    }
...
string generate_monster_data_text(Json data) {
    ostringstream text;

    text << "@ This is auto-generated by monster_data.json.\n";
    ...
    text << "#include \"constants/monster.h\"\n";
    text << "#include \"constants/type.h\"\n";
    text << "#include \"constants/walkable_tile.h\"\n";
    text << ".global gMonsterData\n";
    text << "gMonsterData:";
    for (Json data_entry : data.array_items()) {
        text << read_json_string(data_entry["name"], 4)
            << read_json_string(data_entry["category"], 4)
            << read_json_int(data_entry["overworldPalette"], 1)
            << read_json_int(data_entry["bodySize"], 1)
            << "\n.2byte 0"
            << read_json_int(data_entry["movementSpeed"], 4)
            << read_json_binary(data_entry["dialogueSprites"], 2)
            << read_json_bool(data_entry["unk12"])
            << read_json_string_array(data_entry["types"], 1, 2)
            << read_json_string(data_entry["movementType"], 1)
            << read_json_string(data_entry["friendArea"], 1)
            << read_json_string_array(data_entry["abilities"], 1, 2)
            << read_json_int(data_entry["shadowSize"], 1)
            << read_json_int(data_entry["unk1A"], 1)
            << read_json_int(data_entry["regenSpeed"], 1)
            << read_json_bool(data_entry["canMove"])
            << read_json_int(data_entry["chanceAsleep"], 1)
            << read_json_int(data_entry["baseHP"], 2)
            << read_json_int(data_entry["expYield"], 4)
            << read_json_int_array(data_entry["baseAtkSpAtk"], 2, 2)
            << read_json_int_array(data_entry["baseDefSpDef"], 2, 2)
            << read_json_int(data_entry["weight"], 2)
            << read_json_int(data_entry["size"], 2)
        ...
        if (data_entry["preEvolution"]["evolveType"] == "EVOLVE_TYPE_ITEM") {
            text << read_json_string(data_entry["evolutionRequirements"]["mainRequirement"], 2);
        } else {
            text << read_json_int(data_entry["evolutionRequirements"]["mainRequirement"], 2);
        }

        text << read_json_int(data_entry["evolutionRequirements"]["additionalRequirement"], 2)
            << read_json_int_array(data_entry["dexInternal"], 2, 2)
            << read_json_int(data_entry["recruitRate"], 2)
...
string generate_item_data_text(Json data) {
    ...
    for (Json data_entry : data.array_items()) {
        text << read_json_string(data_entry["name"], 4)
            ...
            << read_json_int(data_entry["spriteID"], 1)
            << "\n.2byte 0"
...
string generate_move_data_text(Json data) {
    ...
    text << "#include \"constants/move_category.h\"\n";
    ...
    for (Json data_entry : data.array_items()) {
        text << read_json_string(data_entry["name"], 4)
            << read_json_int(data_entry["basePower"], 1)
            << "\n.byte 0"
            ...
            << read_json_int(data_entry["basePP"], 1)
            << read_json_int(data_entry["aiWeight"], 1)
            << read_json_int(data_entry["accuracy1"], 1)
            << read_json_int(data_entry["accuracy2"], 1)
            << read_json_int(data_entry["aiConditionRandomChance"], 1)
            << read_json_int(data_entry["numberOfChainedHits"], 1)
            << read_json_int(data_entry["maxUpgradeLevel"], 1)
            << read_json_int(data_entry["critChance"], 1)
            << read_json_bool(data_entry["affectedByMagicCoat"])
            << read_json_bool(data_entry["isSnatchable"])
...
string get_compressed_move_id(string move_macro) {
    ostringstream text;
    text << "\n";
    int move_id = get_move_id(move_macro);
    if (move_id >= 0x80) {
        int move_id_2 = move_id & 0x7F;
        move_id = (move_id >> 7) + 0x80;
        text << ".byte " << std::to_string(move_id) << "\n.byte " + std::to_string(move_id_2);
    } else {
        text << ".byte " << std::to_string(move_id);
    }
    return text.str();
...
string generate_learnset_data_text(Json data) {
    ostringstream text;

    text << "@ This is auto-generated by learnset_data.json.\n";
    for (Json data_entry : data.array_items()) {
...
int main(int argc, char *argv[]) {
    ...
    if (mode == "map") {
        ...
        if (mode == "groups") {
            ...
            process_json(filepath, "monster_data.inc", generate_monster_data_text);
        } else if (mode == "item") {
            process_json(filepath, "item_data.inc", generate_item_data_text);
        } else if (mode == "move") {
            process_json(filepath, "move_data.inc", generate_move_data_text);
        } else if (mode == "trap") {
            process_json(filepath, "trap_data.inc", generate_trap_table_text);
        } else if (mode == "dungeon_item") {
            process_json(filepath, "dungeon_item_data.inc", generate_dungeon_item_encodings_text);
        } else if (mode == "learnset") {
            process_json(filepath, "learnset_data.inc", generate_learnset_data_text);
        } else {
            FATAL_ERROR("ERROR: <mode> must be 'layouts', 'map', 'groups', 'dungeon', 'floor', 'monster', 'item', 'move', 'trap', 'dungeon_item', or 'learnset'.\n");
        }
    }
